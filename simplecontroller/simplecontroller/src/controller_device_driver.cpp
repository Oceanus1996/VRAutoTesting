...
MyControllerDeviceDriver::MyControllerDeviceDriver( vr::ETrackedControllerRole role )
{
	...
	vr::VRSettings()->GetString( my_controller_role_ == vr::TrackedControllerRole_LeftHand ? my_controller_left_settings_section : my_controller_left_settings_section,
...
vr::DriverPose_t MyControllerDeviceDriver::GetPose()
{
	...
	//pose.vecPosition[ 0 ] = position.v[ 0 ] + x;
	//pose.vecPosition[ 1 ] = position.v[ 1 ] + y;
	//pose.vecPosition[ 2 ] = position.v[ 2 ] + z;
	pose.vecPosition[0] =  x;
	pose.vecPosition[1] =  y;
	pose.vecPosition[2] =  z; 
	//DriverLog(" simple controller position x: %f y:%f z:%f, pos.x:%f pos.y:%f pos.z:%f", x,y,z,pose.vecPosition[0],pose.vecPosition[1],pose.vecPosition[2]);
...
bool MyControllerDeviceDriver::CompareAct(float x, float y, float z, bool click, bool press, bool up) {
	bool modify = false;

	if (m_OldData.click != click) {
		DriverLog("modify = true !");
		modify = true;
	}

	if (m_OldData.triggerdown != press)
		modify = true;
	if (m_OldData.triggerup != up)
		modify = true;
	return modify;
...
//-----------------------------------------------------------------------------
void MyControllerDeviceDriver::MyRunFrame(datastruct* data)
{
	// update our inputs here
	m_pData = data;

	bool differentAct = CompareAct(m_pData->posx, m_pData->posy, m_pData->posz, m_pData->click, m_pData->triggerdown, m_pData->triggerup);

	if ( m_pData && differentAct) {
		bool click = m_pData->click;
		if (click) {
			vr::VRDriverInput()->UpdateBooleanComponent(input_handles_[MyComponent_trigger_click], click, 0.0);
			std::this_thread::sleep_for(std::chrono::milliseconds(15));
			vr::VRDriverInput()->UpdateBooleanComponent(input_handles_[MyComponent_trigger_click], false, 0.0);
			DriverLog("UpdateBooleanComponent MyComponent_trigger_click: %s ", click ? "True" : "False");
		}
		bool tdown = m_pData->triggerdown;
		if (tdown) {
			vr::VRDriverInput()->UpdateBooleanComponent(input_handles_[MyComponent_trigger_click], true, 0.0);
		}
		bool tup = m_pData->triggerup;
		if (tup) {
			vr::VRDriverInput()->UpdateBooleanComponent(input_handles_[MyComponent_trigger_click], false, 0.0);
		}

		m_OldData.click = m_pData->click;
		m_OldData.triggerdown = m_pData->triggerdown;
		m_OldData.triggerup = m_pData->triggerup;
		DriverLog("m_ptrOldData_click: %s , triggerdown:%s , triggerup: %s ", m_OldData.click ? "True" : "False", m_OldData.triggerdown? "True" : "False", m_OldData.triggerup ? "True" : "False");
